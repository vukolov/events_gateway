...
class MetricEventsProcessor:
    ...
    def send_to_downstream(self, metric_event: MetricEvent, metric_repo: AbstractMetricRepo) -> None:
        metric_event = self._add_configuration_data(metric_event, metric_repo)
        topic = self._chose_storage_topic(metric_event)
        self._metric_events_storage_session.save_event(metric_event, topic)

    def _add_configuration_data(self, metric_event: MetricEvent, metric_repo: AbstractMetricRepo) -> MetricEvent:
        if not metric_event.metric_group_id or not metric_event.measure_frequency_id:
            ...
            # metric_entity = metric_repo.get_by_uuid(metric_event.metric_uuid) # type: MetricEntity
            # metric_group_entity = self._entities_storage_session.get_metric_group(metric_entity.group_id)
            # metric_event.metric_group_id = metric_group_entity.id
